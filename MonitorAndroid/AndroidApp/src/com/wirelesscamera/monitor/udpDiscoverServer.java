...
public class udpDiscoverServer {
    ...
    public udpDiscoverServer(int sPort)
    {
    	ScanPort=sPort;
    	try {
			ds=new DatagramSocket(ScanPort);
                 ...
                 if((sendByte=sendQueue.poll())!=null)
                 {
            		...
            		try {
            			...
            			while(i>4096)
            			{
            				...
            				index+=1024;
            				i-=1024;
            				DatagramPacket sendDp=new DatagramPacket(send,send.length, InetAddress.getByName("255.255.255.255"),
            						ScanPort);
            				ds.send(sendDp);
    	...
    	public void run()
    	{
    		 while(flag)
    		 {
    			 try {
	                ...
	                process.OnReceiveByte(send, dp.getPort(),dp.getAddress().toString());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
